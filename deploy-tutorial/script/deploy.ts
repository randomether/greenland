import { ethers, defender } from "hardhat";

async function main() {
  const sml = await ethers.getContractFactory("GREENLAND");

  const upgradeApprovalProcess = await defender.getUpgradeApprovalProcess();

  if (upgradeApprovalProcess.address === undefined) {
    throw new Error(`Upgrade approval process with id ${upgradeApprovalProcess.approvalProcessId} has no assigned address`);
  }
  // Generate a unique salt for deterministic deployment
  const salt = ethers.id("GREENLAND-Deployment-20240510-V1");

  const deployment = await defender.deployProxy(sml, [5, upgradeApprovalProcess.address], { initializer: "initialize", initialOwner: upgradeApprovalProcess.address, salt: salt });

  await deployment.waitForDeployment();

  console.log(`Contract deployed to ${await deployment.getAddress()}`);
}

// We recommend this pattern to be able to use async/await everywhere
// and properly handle errors.
main().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});